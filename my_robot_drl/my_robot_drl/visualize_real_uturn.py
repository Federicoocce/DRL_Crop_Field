...
def plot_all_waypoints(waypoints, plant_rows):
    """
    Creates a scatter plot of all waypoints and plant rows, annotating waypoints
    with their index. Boundary waypoints are highlighted.
    """
    if not waypoints:
    ...
    # Plot Plant Rows
    if plant_rows:
        # Plot all plants with a single call for legend purposes
        first_row = np.array(plant_rows[0])
        if first_row.size > 0:
             ax.scatter(first_row[:, 0], first_row[:, 1], c='green', s=15, alpha=0.8, label='Plants', marker='.')
        for row in plant_rows[1:]:
            row_arr = np.array(row)
            if row_arr.size > 0:
                ax.scatter(row_arr[:, 0], row_arr[:, 1], c='green', s=15, alpha=0.8, marker='.')

    # Separate waypoints for coloring based on their role
    boundary_wps = [wp for wp in waypoints if wp['is_lane_boundary']]
    regular_wps = [wp for wp in waypoints if not wp['is_lane_boundary']]

    # Plot regular waypoints, colored by lane
    if regular_wps:
        reg_x = [wp['x'] for wp in regular_wps]
        reg_y = [wp['y'] for wp in regular_wps]
        lane_indices = [wp['original_lane_index'] for wp in regular_wps]
        num_lanes = len(set(wp['original_lane_index'] for wp in waypoints))
        colors = plt.cm.jet(np.linspace(0, 1, num_lanes))
        lane_colors = [colors[idx] for idx in lane_indices]
        ax.scatter(reg_x, reg_y, c=lane_colors, s=50, label='Lane Waypoints', zorder=3)

    # Plot boundary waypoints with a distinct marker
    if boundary_wps:
        bound_x = [wp['x'] for wp in boundary_wps]
        bound_y = [wp['y'] for wp in boundary_wps]
        ax.scatter(bound_x, bound_y, color='red', s=120, marker='*', zorder=5, label='End-of-Row Waypoints')

    # Annotate all waypoints with their original index
    for i, wp in enumerate(waypoints):
        ax.text(wp['x'] + 0.05, wp['y'], str(i), fontsize=9, zorder=4)

    ax.set_title('Overview of All Waypoints and Plant Rows')
...
def main():
    ...
    # 2. Plot all waypoints and plant rows for a general overview
    plot_all_waypoints(master_waypoints, plant_rows)
